...
const VideosBox = ({ info }) => {

  const { snippet,statistics  } = info;
  const { channelTitle, title, thumbnails, publishedAt} = snippet;

  return (
    <div className='p-2 m-2 w-72 flex flex-col justify-center items-center'>
      ...
      <ul className='flex justify-start items-start'>
        ...
        <div>
          ...
          <li className='text-gray-500 text-[13px]'>{statistics?.viewCount ? formatCompactNumber(statistics?.viewCount) : 0} views  {(Math.abs(new Date(publishedAt) - new Date()) / (60 * 60 * 24 * 1000)).toFixed(0)} days ago</li>
        </div>
      </ul>
    </div>
  )
